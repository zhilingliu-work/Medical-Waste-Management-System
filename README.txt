...
1. **根帳號 (root)**[cite: 1]：
   * 系統最高權限管理員[cite: 1]。
   * 獨佔可管理 Django Administration（本系統帳號管理底層框架）[cite: 1]。
   * 唯有具備伺服器管理權限的使用者，才能動態管理一般事業廢棄物之欄位資訊[cite: 1]。
2. **管理者 (manager)**[cite: 1]：
   * 負責系統營運配置與高階審核，可操作【後台管理】與所有基本配置物件設定[cite: 1]。
3. **行政人員 (staff)**[cite: 1]：
   * 負責院內日常行政作業，包括過磅與載運紀錄查詢、結算載運單、生成統計報表[cite: 1]。
4. **登錄者 (loginer)**[cite: 1]：
   * 日常資料維護人員，協助處理基礎系統資料登記[cite: 1]。
5. **匯入者 (importer)**[cite: 1]：
   * 負責批次外部數據與歷史資料匯入系統[cite: 1]。
6. **外部人員 (outer)**[cite: 1]：
   * 現場實體清運與作業人員[cite: 1]。
   * 負責在現場操作【行動裝置登記】與【QR code列印】[cite: 1]。
...
### 系統帳號管理
* **連動式檢視**：點擊左側角色群組與使用者列表，右側詳細資料即時連動呈現[cite: 1]。
* **動態新增與權限分組**：提供完整表單輸入使用者帳號、密碼及指定權限等級（身分組）[cite: 1]。
* **列表管理**：列表依身分組清晰歸類，支援一鍵刪除[cite: 1]。

### 廢棄物種類和部門管理
* **雙欄整合配置**：左欄管理廢棄物種類（名稱、單位），右欄管理部門資訊（代碼、名稱）[cite: 1]。
* **行內新增**：點擊【新增】提供空白輸入列，支援使用「分號」一次輸入多個物件資訊，再點擊打勾儲存[cite: 1]。
* **行內編輯/刪除**：提供一鍵切換可更改狀態、保存、取消或單筆垃圾桶刪除[cite: 1]。
* **批次刪除**：勾選複數項目後，上方按鈕會動態切換為「批次刪除」[cite: 1]。
* **關鍵字查詢**：提供頂部查詢列，輸入代號或名稱即可即時模糊篩選[cite: 1]。

### 定點和機構管理
* **實體定點維護**：左欄管理院內所有實體過磅定點（代碼、定點名稱，如醫療大樓一樓之一）[cite: 1]。
* **外部合作機構**：右欄管理委外清除機構與處理機構，支援行內編輯、批次勾選刪除及關鍵字即時檢索[cite: 1]。

### 警報紀錄管理
* **主動式異常警報**：即時追蹤過磅流程產生的重量異常與申報頻率異常[cite: 1]。
* **複合條件篩選**：支援依發生日期（起/迄）、警報名稱、警報類型及產出部門進行複合式查詢，並提供一鍵「↺」重置功能[cite: 1]。
* **定位追蹤與標記**：檢視警報詳情時，點擊【定位紀錄】按鈕，系統會自動跳轉至對應過磅頁面，並對該異常紀錄實施「黃色高亮閃爍 3 秒」的精準追蹤[cite: 1]。
* **警報條件設定門檻**：可動態設定「廢棄物產出量監控」的上下限重量閥值，及各部門的「過秤頻率監控」（每日過秤次數下限）[cite: 1]。
* **趨勢折線圖**：可自由切換特定年份或月份，顯示警報次數的時間軸走勢[cite: 1]。
* **進度條監控**：展示各部門昨日應過磅與實際過磅完成次數的比例，支援輸入關鍵字過濾部門進度條[cite: 1]。
...
### 行動裝置登記
* **相機條碼解碼**：整合行動裝置鏡頭，一鍵喚起相機掃描廢棄物袋上的 QR code，自動解碼並在畫面上秒速帶出部門、種類、人員與打包時間[cite: 1]。
* **藍牙連線**：透過網頁端直接配對自行組裝的 ESP34 實體電子磅秤裝置[cite: 1]。
* **重量讀取**：點擊【重量紀錄】按鈕即時從磅秤硬體獲取重量並呈現在畫面上[cite: 1]。
* **遠端控制功能**：支援「秤盤一鍵歸零」、「多點參數校正（輸入已知公克重量重新計算校正因子）」、「單位動態切換（公斤 KG / 公克 G）」以及「遠端修改磅秤裝置名稱（限 20 字元內）」[cite: 1]。
* **一鍵安全上傳**：確認無誤後，點擊【送出紀錄】即可將完整的重量、條碼、定點資訊整合封包上傳資料庫，並自動重置表單[cite: 1]。

### 過磅紀錄管理
* **全局看板**：頂部區塊顯示總過磅筆數、昨日過磅重量(kg)、已載運重量(kg)與未載運重量(kg)等營運指標[cite: 1]。
* **刪除防護鎖定**：操作欄設有安全隔離鎖[cite: 1]。
* **狀態辨識**：已關聯載運單（已清運）的過磅紀錄顯示鎖頭圖示禁止刪除；未載運的過磅紀錄方顯示垃圾桶允許刪除[cite: 1]。
* **一鍵結算載運**：勾選複數未載運過磅紀錄，點擊【建立載運單(結算)】，可在懸浮視窗中選擇合作的清理公司與處理公司，一鍵打包轉換成載運單[cite: 1]。
* **多元匯出與列印**：支援將篩選後或手動勾選的過磅資料，即時轉存為高解析度 PDF、PNG 或標準 Excel 表單，並支援直接呼叫列印預覽[cite: 1]。

### 載運紀錄管理
* **複合式回溯**：提供載運日期區間、清理/處理機構名稱的複合查詢[cite: 1]。
* **安全反結算**：如載運有誤，可勾選該載運紀錄點擊【取消載運】，將其整單作廢，並自動將該批過磅紀錄狀態還原至「未載運」狀態以供重新分配，安全機制嚴密[cite: 1]。
* **嵌套明細檢視**：點擊【查看】可開啟懸浮視窗，顯示該載運單打包的所有子過磅紀錄明細[cite: 1]。

### 報表管理 (部門)
* **動態自訂圖表**：提供視覺化自訂報表產生器[cite: 1]。
* **版面拖曳**：使用者可自訂建立多個圖表，並支援在畫面上以拖拉式調整圖表的排列與匯出順序[cite: 1]。
* **線段疊加**：在單個圖表中，點擊【新增線段】可自由疊加不同的資料源、時間區段及廢棄物種類[cite: 1]。
* **細部渲染**：提供「顯示圖表數值」、「顯示完整表格格線」與「顯示原始資料表格」等功能開關[cite: 1]。
* **多檔案匯出**：圖表渲染生成後，支援單鍵匯出為高畫質 PNG 圖片、PDF 技術文件、或是含有完整原始資料分頁的 Excel 試算表[cite: 1]。
